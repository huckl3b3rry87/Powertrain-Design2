...
people = 70*4; % 70 kg per passenger
     ...
     Af = 2;            % m^2 - Frontal Area
     rwh = 0.2794;           % m - Wheel Radius
     nem = 0.95;           % Motor Average Efficiency
     delta = 1+ Iw/(m*rwh^2);       % Mass Factor
...
for x3 = 1:x3_length
    ...
    figure(2);
    plot(V/mph_mps,Eng_Power/1000,'color',C(x3,:),'linewidth',3)
    hold on
...
figure(1);
plot(V/mph_mps,Max_Eng_Tractive_Effort/1000,'k*','markersize',6,'markerweight','bold')
hold on
