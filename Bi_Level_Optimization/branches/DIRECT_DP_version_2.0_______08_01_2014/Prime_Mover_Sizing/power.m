...
%Base Power 1
alpha = 0;
Peng1 = (m*g*Frr*ones(size(V)) + 0.5*rho*Cd*Af*V.^2 + m*g*alpha*ones(size(V))).*V/(1000*nt);

%Base Power 2
alpha = 0.05;
Peng2 = (m*g*Frr*ones(size(V)) + 0.5*rho*Cd*Af*V.^2 + m*g*alpha*ones(size(V))).*V/(1000*nt);
