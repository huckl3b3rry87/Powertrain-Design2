...
for t = 1:cyc_data.time_cyc
    ...
    % Add in the Engine state
    table_x1 = repmat(table_x1,[1,1,1,1,x2_length]);
    table_x1 = permute(table_x1,[1 5 2 3 4]);
    table_x1(:,1,:,:,:) = single(zeros(x1_length,x3_length,u1_length,u2_length));  
    table_x1(:,2,:,:,:) = single(ones(x1_length,x3_length,u1_length,u2_length));  
...
if RUN_TYPE.emiss == 1
    emission.NOx = sum(sim.NOx)/total_distance_mile*1000;   % dt is 1 - grams
    emission.HC = sum(sim.HC)/total_distance_mile*1000; %mg/mile
    emission.CO = sum(sim.CO)/total_distance_mile*1000;
else
    emission = NaN;
end
