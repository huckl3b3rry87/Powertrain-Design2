            ...
            for x2 = 1:x2_length             % Current Engine State
                ...
                for u3 = 1:u3_length         % Engine Control
                    ...
                    if Pd(t) < 0 && ENG_state_n == 1 % Braking and Running Engine - 
                        ...
                        Tm_c = Tw(t)/(x3_n*FD*G)*ones(size(Te_c)); % Engine is not connected to motor
                    else
...
fail_inner_Pbatt = zeros(DIM,time_cyc);
...
for t = 1:1:time_cyc
    ...
    if Pd(t) < 0 && ENG_state_n == 1
        ...
        Taux = Paux/We_c;
        Te_c = Taux;
        Tm_c = Tw(t)/(GEAR_n*FD*G);  % Engine is disconnected
    else
    ...
    if Wm_c ~= 0
        Tm_actual = Pbat/(Wm_c*eff_m^SIGN) - Paux/Wm_c;
    else
        Tm_actual = 0;
    end
