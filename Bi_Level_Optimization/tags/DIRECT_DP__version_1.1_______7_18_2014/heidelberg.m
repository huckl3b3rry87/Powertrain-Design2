...
load RAMP; cyc_name = 'RAMP_penalty';
            ...
            for u1 = 1:u1_length
                 for u2 = 1:u2_length
                      for u3 = 1:u3_length
                        ...
                        % Let the controls move the states!!
...
%     else
%         J_temp = SOC_State_Penalty;
%     end
% Should I add in L on the first run "last"
    % New Minimum Method
...
tic;
    ...
    %     Tm_c(Tm_c < -Tm_max) = -Tm_max;  % Should we do this?? - In the
    %     optimization, we should do this for everything, so that it cannot
    %     fail. Then check and see if it failed.
...
set(AX(1),'XLim',[0 t]);
set(AX(2),'XLim',[0 t]);
set(AX(1),'YLim',[0 1.1*max(inst_fuel)]);
