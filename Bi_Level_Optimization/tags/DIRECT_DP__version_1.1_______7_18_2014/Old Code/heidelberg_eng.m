...
u1_grid = [0:20:140]';          % Engine Torque [N-m]
...
%                 if ( u2 == u2_length && Pd(t) > 0 && x2 == 1 && SOC_c == 0.5)
%                     Pbat_off = Pbat;
%                 end
%                 if (u2 == u2_length  && Pd(t) > 0 && x2 == x2_length && SOC_c == 0.5)
%                     t
%                     SOC_save(:,:,3)
%                     Pbat_on = Pbat
%                     Pbat_off
...
%                 end
                ...
                % u3 does not affect the next SOC, but may need to add in
                % [u3] dimension - WRONGO!! - ADD u3 Dimension!!!!
                % Make sure that you have enough SOC_save ect..
        ...
        SOC_final = repmat(SOC_save,[1 1 1 2]);  % WRONGO!!! Should Already have the u3 dimension!!!
        ...
        SOC_soft_constraint = zeros(size(SOC_save));    %[x2]x[u1]x[u2]
        ...
        soft = repmat(SOC_soft_constraint,[1,1,1,2]);   %[x2]x[u1]x[u2]x[u3]
        table_L(x1,:,:,:,:) = temp + soft;
