...
for FD = 1:0.05:8
    dvar.FD = FD;
    [Sim_Grade, F_max_6, V_max_6, RR ] = Engine_Power_Sizer( param, vinf, dvar, test ); 
...
test = 2;
V_test = 20*param.mph_mps;

iii = 1;
for fc_trq_scale = 0.1:0.01:1.75
...
dvar.mc_trq_scale = 0.75;  % Just leave this and see where it goes...maybye?? - Got to get things going!!
mc_max_pwr_kW =  dvar.mc_trq_scale*vinf.mc_max_pwr_kW;
dvar.module_number = ceil(4*mc_max_pwr_kW*1000*Rint_size/(Voc_size^2));
Manipulate_Data_Structure;
EE = 1;
...
for i = 1:(length(V_0-2)+1)
    if i == (length(V_0)+1)
        ...
        [ PASS, Sim_Variables ] = Acceleration_Test(V_0_n,V_f_n, Acc_Final_temp, dt_2, param, vinf, dvar, TYPE);
    else
