...
load AA_final;
cyc_name = 'HWFET_AA';
temp_spd = [temp_spd ; cyc_mph(:,2)];
time = [1:length(temp_spd)]';
clear cyc_mph
cyc_mph(:,2) = temp_spd;
cyc_mph(:,1) = time;
save('CYC_HWFET_AA','cyc_mph');
plot(cyc_mph(:,1),cyc_mph(:,2),'g','linewidth',3),grid
% legend('HWFET and Natralistic Ann Arbor Cycle')
ylabel('Speed (mph)','fontWeight','bold','fontSize',12)
xlabel('time (sec)','fontWeight','bold','fontSize',12);
set(gca,'fontSize',12,'fontWeight','bold')
title('HWFET and Natralistic Ann Arbor Cycle','fontWeight','bold','fontSize',16)
