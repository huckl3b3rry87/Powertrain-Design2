...
for t = 1:cyc_data.time_cyc
            ...
            for x2 = 1:x2_length             % Current Engine State
                ...
                for u3 = 1:u3_length         % Engine Control
                    ...
                    if  ENG_state_c == 0 && u3 == 2   % Engine will be turned on - Done later in the code..this is the same thing though.
                        Eng_Penalty = dvar.fc_trq_scale*10;
                    else
                        Eng_Penalty = 0;
                    end
                    ...
                    Te_max =  interp1(vinf.eng_consum_spd,vinf.eng_max_trq,We_c)*ones(size(Te_c));
                    ...
                    if ENG_state_n == 0
                        Pbat = Pbat + vinf.Paux;    % Draw Aux Power From Battery
                    end
    ...
    infeasible_We(:,:,:,:,2) = (We_save(:,:,:,:,2) < 450*param.rpm2rads) | (We_save(:,:,:,:,2) > vinf.W_eng_max);
...
for t = 1:1:cyc_data.time_cyc
    ...
    sim.J(t) =  interp1(x1_grid,J_STAR(:,ENG_state_id,GEAR_id),SOC_c,'nearest');
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    if cyc_data.Pd(t) < 0 && ENG_state_n == 1
        We_c = vinf.optimal_eng_spd;
        Taux = vinf.Paux/We_c;
        Te_c = Taux;
    ...
    elseif We_c < vinf.W_eng_min
        ...
        if We_c < 800*param.rpm2rads &&  ENG_state_n == 1 % Engine is on
            Fail_We = 1;
        else
    ...
    Te_max = interp1(vinf.eng_consum_spd,vinf.eng_max_trq,We_fuel);
    %                         ~Engine Torque~
    if Te_c > Te_max
        Te_fuel = Te_max;
        Fail_Te = 1;
    ...
    if ENG_state_n == 0;
        Pbat_terminal = Pbat + vinf.Paux;   % If Pbat is positive, we are adding to the system and taking away from the battery SOC
