            ...
            for x2 = 1:x2_length             % Current Engine State
                ...
                for u3 = 1:u3_length         % Engine Control
                    ...
                    if Pbat >= 0
                        ...
                        infeasible_Pbatt(:,x2,x3,:,u2,u3) = zeros(size(Pbat));    % Can brake to make up the difference
...
SOC_CORRECTION = 0.5:0.05:0.6;  % Extract Solutions From the Middle - SHould only need to extract one. Can fail it if it does not meet standards

% Pre-Allocate for Speed
DIM = length(SOC_CORRECTION);
fail_inner_SOC = zeros(DIM,time_cyc);
fail_inner_Te = zeros(DIM,time_cyc);
fail_inner_We = zeros(DIM,time_cyc);
fail_inner_Tm = zeros(DIM,time_cyc);
fail_inner_Wm = zeros(DIM,time_cyc);

for u = 1:DIM
    SOC_c = SOC_CORRECTION(u);
    if RUN_TYPE == 0
        fprintf('-------------------------------------------------\n');
...
%         Pbat = Pm_c;
    ...
    Pbat = Pm_c;
        ...
        if Wm_c ~= 0
            Tm_actual = Pbat/(Wm_c*eff_m^SIGN);
        else
            Tm_actual = 0;
        end
        T_brake = Tm_c -  Tm_actual; % Positive then the actual motor torque supplied was less than that which was controlled
        % Motor Efficeincy should be updated - so should the battery
        % power!- so this will affect SOC and needs to go into DP as
        % well!-should not affect that much...
...
FAIL = ((fail_outer_Tm + fail_outer_Wm + fail_outer_Te + fail_outer_We + fail_outer_SOC) ~= 0);
...
figure(33);
plot(cyc_time, W_mot.*T_brake_sim)
figure(34); plot(ess_soc,ess_max_pwr_dis)